...
""" Note that this file is a downgraded version and th edyanamics of the system and the actual working code cannot be submitted at the moment as it is sa crucial part of a publication in process"""
""" Still this file can run and produce results(just not satisfactorily) using  q learning method"""
"""THIS FILE HAS mdp FORMULATION FOR  A CUSTOM MANIPULATION DEVICE AND Q Learning applied to it"""
""" Please delete above comments including this one and run the code"""
...
class FrictionFinger :
    """
try other vozulizations
try to get goal closer
exploration rate
...
"""
    ...
    def step(self, action):
        ...
        #rew= lambda st: 1 if st[0]==self.d1_d and st[1]==self.d2_d else 0
        ...
        if self.state[0:2] == [self.d1_d, self.d2_d] :
            self.done =True
        else:
            self.done= False
            self.state=[max(min(self.state[0],12),0),max(min(self.state[1],12),0),self.state[2]]
